...
void Automobile::emptyFuel(void) { fuelInTank = 0; }
...
void Automobile::addFuel(double _liters) {
  ...
  if (fuelInTank > 50) {
    ...
    snprintf(message, sizeof(message),
             "The %s %d %s %s is full of gas. Discarding the rest...\n",
...
void Automobile::drive(double _distance) {
  ...
  if (fuelInTank < 0) {
    ...
    char message[64];
    snprintf(message, sizeof(message),
             "The %s %d %s %s has no gas left in the tank\n", colour.c_str(),
             year, make.c_str(), model.c_str());
    Log(ERROR, __FILE__, __func__, __LINE__, message);
  }
}

void Automobile::displayReport() {
  cout << "The " << colour << " " << year << " " << make << " " << model
       << " has " << fuelInTank << " left in the tank" << endl;
}
