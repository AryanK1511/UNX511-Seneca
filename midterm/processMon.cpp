...
int main() {
  // Question 2: What happens if the following is executed from the command
  // line: "pkill -2 processMon"
  ...
  for (int i = 0; i < NUM && isParent; ++i) {
    ...
    if (childPid[i] == 0) { // the child
      isParent = false;
      // Question 5: What function in the kernel is called when read is called
      // here?
      read(fd, key, MAXBUF);
      close(fd);
      execlp(args[i], args[i], (const char *)key, NULL);
      // Question 6: Why is it an error if the program continues beyond
      // execlp()?
  ...
  if (isParent) {
    sleep(1);
    // Question 7: What function in the kernel is called when ioctl is called
    // here?
  ...
  for (int i = 0; i < NUM; ++i) {
    // Question 8: What function in the child processes is called when they
    // receive a SIGINT?
  ...
  while (isRunning && pid >= 0) {
    ...
    for (int i = 0; i < NUM && isParent && !isFound; ++i) {
      if (pid == childPid[i]) {
        ...
        write(fdLog, buf, len);
        flags = flags & ~O_EXCL;
        // Question 11: What is processMon attempting to do with the log file
        // now?
        fcntl(fdLog, F_SETFL, flags);
        close(fdLog);
        childPid[i] = fork();
